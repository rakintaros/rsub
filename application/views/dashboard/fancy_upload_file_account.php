...
<html lang="en">
  <head>
	...
	<body>
		<div style="min-height:400px;">
			...
			<form class="form-inline" enctype="multipart/form-data" action="#" method="post">
				...
				<input type="hidden" name="account_point" id="account_point" value="<?=$account_point?>">
	...
	$(function() {
		...
		$(document).on('click','.del_file_account', function(e) {
			if (confirm('ยืนยันการลบรายการ?')) {
				var f_id = $(this).attr("file");
				var p_id = $(this).attr("fproject");
				$.post( "<?=base_url('dashboard/ajax_accountfile_del')?>", { account_id: p_id, file_id: f_id }).done(function( data ) {
					console.log(data);
					loadTable();
				});
			}	
		...
		function loadTable(){
			...
			var table_body = '';
			$.getJSON( "<?=base_url('dashboard/ajax_accountfile')?>", { account_id: '<?=$account_id?>', account_point: '<?=$account_point?>' } ).done(function( data ) {
				var i=0;
				$.each( data, function( key, val ) {
					...
					table_body +='	<td><button class="btn btn-danger btn-sm del_file_account" fproject="'+val.file_account_id+'" file="'+val.file_id+'"><i class="mdi mdi-delete"></i></button></td>';
					table_body +='</tr>';
				});
		...
		$('#upload').on('click', function () {
			...
			$('#msgLoad').show();
					...
					form_data.append("account_point", $('#account_point').val());
                    $.ajax({
                        url: '<?=base_url("dashboard/account_ajax_uploads")?>', // point to server-side PHP script 
