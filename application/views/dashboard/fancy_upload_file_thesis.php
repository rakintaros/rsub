...
<html lang="en">
  <head>
	...
	<body>
		<div style="min-height:400px;">
			...
			<form class="form-inline" enctype="multipart/form-data" action="#" method="post">
				...
				<input type="hidden" name="file_thesis_id" id="file_thesis_id" value="<?=$file_thesis_id?>">
	...
	$(function() {
		function loadTable(){
			var table_header = '<table class="table"><thead class="bg-secondary"><tr><td>#</td><td>File Name</td><td>Size</td><td></td></tr></thead><tbody>';
			var table_footer = '</tbody></table>';
			var table_body = '';
			$.getJSON( "<?=base_url('dashboard/ajax_thesisfile')?>", { file_thesis_id: '<?=$file_thesis_id?>'} ).done(function( data ) {
				var i=0;
				console.log(data);
				...
				$.each( data, function( key, val ) {
					...
					table_body +='<tr>';
					table_body +='	<td>'+i+'</td>';
					table_body +='	<td><a href="<?=base_url()?>uploads/thesis/'+val.file_path+'" target="_blank">'+val.file_name+'</a></td>';
					table_body +='	<td>'+val.file_size+'</td>';
					table_body +='	<td><button class="btn btn-danger btn-sm del_file" fsource="'+val.file_thesis_id+'" file="'+val.file_id+'"><i class="mdi mdi-delete"></i></button></td>';
					table_body +='</tr>';
		...
		$("#upload").on("click", function () {
			...
			form_data.append("file_thesis_id", $("#file_thesis_id").val());
			$.ajax({
				url: '<?=base_url("dashboard/thesis_ajax_uploads")?>', // point to server-side PHP script
		...
		$(document).on("click", ".del_file", function (e) {
			if (confirm("ยืนยันการลยรายการ?")) {
				...
				$.post("<?=base_url('dashboard/ajax_thesisfile_del')?>", { file_thesis_id: s_id, file_id: f_id }).done(function (data) {
					loadTable();
				});
			}
		});
